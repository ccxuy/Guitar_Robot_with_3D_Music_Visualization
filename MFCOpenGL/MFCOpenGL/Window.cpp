...
static CComApp comDlg;
static int midiValue[18];
static HANDLE hThread;

void SyncThreadFunc();
unsigned int __stdcall ComProc(LPVOID lpParameter);
unsigned int __stdcall GraphicProc(LPVOID lpParameter);
...
void ResizeWindowGL(int width, int height)								// 重新设置窗口大小
{
	...
	gluPerspective(45, (float)width/(float)height, 0.1, 3000);			// 设置透视投影
...
void SyncThreadFunc(){
	...
	//创建线程
	//comDlg.InitInstance();
	hThread = (HANDLE)_beginthreadex(NULL, 0, ComProc, (void*)midiValue, 0, NULL);
}

//void *pPM
unsigned int __stdcall ComProc(LPVOID lpParameter)//thread data
{
	int *midiValue = (int*)lpParameter;
	...
	// 给应用程序赋值
	application.className = "COM_COMM";									// 程序类名字
	//application.hInstance = hInstance;									// 程序入口
...
unsigned int __stdcall GraphicProc(LPVOID lpParameter)//thread data
{
	int *midiValue = (int*)lpParameter;
